...
Item {
    ...
    function updateRecentGames() {
        ...
        else
        {
            ...
            console.log( "DEBUG MODE! Hardcoded data. raRecentGames count " + raRecentGames.count );
            updateRARecentGameIndex( 0, true );
            if (raRecentGames.count === 0) {
                noGameText = 'No Game';
            }
            ...
            https.onreadystatechange = function() { 
                if (https.readyState == XMLHttpRequest.DONE) {
                    if (https.status == 200) {
                        var response = https.responseText;
                        //console.log( response );
                        let cheevos = JSON.parse (response );
                        for (var key in cheevos.Achievements) {
                            //console.log( cheevos.Achievements[key].Title );
                            let dateEarned = '';
                            let hardcore = false;
                            if (cheevos.Achievements[key].DateEarned !== undefined) {
                                dateEarned = cheevos.Achievements[key].DateEarned;
                            }
                            if (cheevos.Achievements[key].DateEarnedHardcore !== undefined) {
                                dateEarned = cheevos.Achievements[key].DateEarnedHardcore;
                                hardcore = true;
                            }
                            let timestampEarned = Date.parse(dateEarned);
                            if (isNaN(timestampEarned)) { timestampEarned = 0; }
                            raGameCheevos.append( { "Title": cheevos.Achievements[key].Title,
                                                    "Description": cheevos.Achievements[key].Description,
                                                    "Points": parseInt(cheevos.Achievements[key].Points),
                                                    "BadgeName": cheevos.Achievements[key].BadgeName,
                                                    "DisplayOrder": parseInt(cheevos.Achievements[key].DisplayOrder),
                                                    "DateEarned": timestampEarned,
                                                    "Hardcore": hardcore
                                                  } );
                        }
                        currentGameDetails = { "Title": cheevos.Title,
                                               "ImageIcon": cheevos.ImageIcon,
                                               "ConsoleId": cheevos.ConsoleID, "ConsoleName": cheevos.ConsoleName,
                                               "Genre": cheevos.Genre,
                                               "NumAchievements": parseInt(cheevos.NumAchievements),
                                               "NumAwardedToUserHardcore": parseInt(cheevos.NumAwardedToUserHardcore),
                                               "NumAwardedToUser": parseInt(cheevos.NumAwardedToUser)
                                             };
